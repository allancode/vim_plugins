...
function get_self_extract_header_comment(){
	cat > ${DEST_FILE} << EOF
#! /bin/bash
## self unpackage script header
## created by longbin <beangr@163.com>
...
function get_self_extract_header_content(){
	## change $ --> \$
	cat >>${DEST_FILE} <<EOF
DST_DIR=\$HOME
...
function insert_script_content_to_file(){
...
function create_text_header_to_file(){
	get_self_extract_header_comment
	get_self_extract_header_content
}


function append_self_extract_tail_to_file(){
	read -p "Insert SCRIPT to the file <y/n>? " select
	if [[ "${select}" == "y" ]] ;then
		read -p "Insert the script before/after extract package <b/a>? " select
		if [[ "${select}" == "a" ]] ;then
			echo "restore_package_file" >> ${DEST_FILE}
			insert_script_content_to_file
		elif [[ "${select}" == "b" ]] ;then
			insert_script_content_to_file
			echo "restore_package_file" >> ${DEST_FILE}
		fi
...
function change_self_extract_file_mode(){
	chmod a+x ${DEST_FILE}
	echo "Done."
	ls -lh ${DEST_FILE}
}

create_text_header_to_file
append_self_extract_tail_to_file
change_self_extract_file_mode
exit 0
